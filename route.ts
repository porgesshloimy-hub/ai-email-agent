...
export async function GET(request: NextRequest) {
  try {
    ...
    if (after) {
      /**
       * Architecture change: no longer fetches/returns
       * `chat_agent_replying` — the typing indicator is now driven
       * entirely by the client's own content-length-based reveal delay
       * (see AgentChatPanel.tsx), not real server status. Generation
       * (lib/agent/chat.ts) also no longer paces itself artificially,
       * so a single poll typically catches the whole reply at once.
       */
